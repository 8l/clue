-- $Id: c.pm 47 2008-09-07 12:32:25Z dtrg $
-- $HeadURL: https://cluecc.svn.sourceforge.net/svnroot/cluecc/clue/pmlib/c.pm $
...
CCOMPILER = "gcc"
CXXCOMPILER = "g++"
CC = "%CCOMPILER% %CBUILDFLAGS% %CDYNINCLUDES:cincludes% %CINCLUDES:cincludes% %CDEFINES:cdefines% %CEXTRAFLAGS% -xc -c -o %out% %in%"
CXX = "%CXXCOMPILER% %CBUILDFLAGS% %CDYNINCLUDES:cincludes% %CINCLUDES:cincludes% %CDEFINES:cdefines% %CEXTRAFLAGS% -xc++ -c -o %out% %in%"
		...
		for line in file:lines() do
			...
			if f then
				for _, path in ipairs(localincludes) do
					local subfilename = path
					if (subfilename ~= "") then
						subfilename = subfilename.."/"
					end
					subfilename = subfilename..f
